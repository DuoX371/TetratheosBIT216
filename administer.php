...
<div class="container">
	<h1>Pending</h1>
	<?php
	if (mysqli_num_rows(getAppointment("confirmed", $_SESSION["currentUser"]["centreName"])) > 0) {
	?>
		<table class="table">
			...
			?>
				<tr>
					...
					<td><button type='button' class='btn btn-primary' data-bs-toggle="modal" data-bs-target="#confirmation" <?php echo "onclick='passModal(this.value)' id='' value='" . $record['vaccinationID'] . "'>" ?> Select</button> </td>
				</tr>
			<?php } ?>

		</table>
	<?php } else {
		echo "There are currently no pending vaccinations to be administered";
	}?>
	<h1 style="padding-top:20px;">Administered</h1>
	<?php
	if (mysqli_num_rows(getAppointment("Administered", $_SESSION["currentUser"]["centreName"])) > 0) {
	?>
	<table class="table">
		<thead class="thead-dark">
			<tr>
	...
	<?php } else {
		echo "There are currently no administered vaccination appointments";
	}
	?>
	<!-- Modal -->
